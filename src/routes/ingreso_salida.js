...
router.get("/getAllIngresos", (req, res) => {
  try {
    ...
    const { id_user_responsable } = req.body;
...
router.post("/getAllIngresosDetailById", (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ where: "ERROR AT: /getAllIngresosDetailById", error });
  }
});

router.post("/ingresoAdd", (req, res) => {
  try {
    const { comentario, id_user_responsable, productos_concat } = req.body;

    let error = false;
    let mensaje_error = "";

    let listaProductos = productos_concat.split("@");
    ...
    mysqlConnection.query(
      ...
      (err, rows, _fields) => {
        if (err) {
          ...
          const [RolState] = rows[0];
          let { state, response } = RolState;
          if (state == 1) {
            const [DataState] = rows[1];
            let { state, message } = DataState;
            if (state == 1) {
...
router.get("/getAllRetirosDetailById", (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ where: "ERROR AT: /getAllRetirosDetailById", error });
  }
});
      ...
      (err, rows, _fields) => {
        if (err) {
          ...
          if (state == 1) {
            ...
            if (state == 1) {
              if (procesado != null) {
                // VALIDAR QUE CUANDO NO HAYA PRODUCTOS BAJO NO ENVIE DATA
                let emailToSend = armarListaCompras(procesado);
                sendEmail(
                  "jhosepromero14@gmail.com",
                  "PRODUCTOS A COMPRAR - STOCK BAJO!!!",
                  emailToSend
                );
              }
