...
INSERT INTO `tb_ingreso_producto` VALUES (1,1,3,5,0,1.5,1.5,0),(2,1,4,10,10,2.5,2.5,1),(3,2,3,10,10,1.5,1.5,1),(4,2,4,20,20,2.5,2.5,1),(7,4,7,10,0,1,1,0),(9,5,7,10,0,1,1,0),(11,6,7,10,10,1,1,1),(13,7,7,10,10,1,1,1),(15,8,7,10,10,1,1,1),(16,9,9,50,50,1.5,1.5,1),(17,10,27,250,250,10,10,1),(18,10,28,100,100,15,15,1),(20,11,7,10,10,1,1,1),(22,12,7,10,10,1,1,1),(24,13,7,10,10,1,1,1),(28,17,27,250,250,10,10,1),(29,17,28,100,100,15,15,1),(30,18,27,250,250,10,10,1),(31,18,28,100,100,15,15,1),(34,21,7,10,10,1,1,1),(36,22,7,10,10,1,1,1),(38,23,7,10,10,1,1,1),(40,24,7,10,10,1,1,1),(42,25,7,10,10,1,1,1),(44,26,7,10,10,1,1,1),(45,27,28,100,100,15,15,1),(46,27,27,5000,5000,1.9,1.9,1),(47,28,27,5000,5000,1.9001139999999999,1.9001139999999999,1),(48,28,28,100,100,15,15,1),(49,29,27,5000,5000,1.9001139999999999,1.9001139999999999,1),(50,29,28,100,100,15,15,1),(51,30,27,5000,5000,1.90015,1.90015,1),(52,30,28,100,100,15,15,1),(53,31,27,5000,5000,1.90015,1.90015,1),(54,31,28,100,100,15,15,1),(55,32,27,1000,1000,3.5,3.5,1),(56,32,28,100,100,15.0045,15.0045,1),(84,62,6,10,0,1.2,1.2,0),(85,63,6,10,0,1.2,1.2,0),(86,64,6,10,3,1.2,1.2,1);
...
INSERT INTO `tb_retiro` VALUES (6,'TEST RETIRO','2022-05-23 05:29:59',3,1);
...
INSERT INTO `tb_retiro_producto` VALUES (3,6,6,27,1.2,1);
...
/*!50003 DROP PROCEDURE IF EXISTS `cautf8AddOrEdit` */;
...
ELSE
    ...
    ELSE
utf8
    	SET @error = 'NO EXISTE LA CATEGutf8
utf8
    END IF;
...
BEGIN
    ...
    WHILE (@cantidad_bucle != 0) DO		
        ...
        SET @id_ingreso = (
            ...
            WHERE TP.id = @id_producto AND TIP.active = 1
        ...
        IF @resto > 0 THEN			
            ...
            VALUES (@id_retiro, @id_producto, _cantidad_retirar, @precio_retiro, 1);
            ...
            SET @cantidad_producto_global = (SELECT TP.cantidad FROM tb_producto TP WHERE TP.id = @id_producto);
        ...
        IF @resto < 0 THEN
			...
			SET @cantidad_bucle = @cantidad_buutf8cantidad_rest_compra; -- mantenemos al bucle restando la cant rest compra, para avanzar con otra compra.
			-- actualizamos la compra restandoutf8 desactivamos POR QUE NECESITAMOS MAS COMPRAS
			UPDATE tb_ingreso_productoutf8
...
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = utf8cs_client */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `inutf8d` */;
/*!50003 SET @saved_cs_client      = utf8cter_set_client */ ;
...
utf8
    WHILE (@contadorProductos <= _canutf8roductos AND @state = 1) DO
utf8
...
BEGIN
	...
	SET @ACCESS := 0;utf8
utf8
    SET @MSG := 'TODO BIEN, HAY DATOSutf8ER!!!';	
...
utf8
utf8
        SELECT * FROM tb_producto proutf8
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `retiroAdd`()
BEGIN
	START TRANSACTION;
...
WHILE (@contadorProductos <= _cantidad_productos AND @state = 1) DO
        ...
        SET @existe_producto = IF( EXISTS
        (SELECT * FROM tb_producto tputf8tp.id = @id_producto),
        1, 0);utf8
utf8
        IF @existe_producto = 1 THEN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `retiroAddV2` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `retiroAddV2`(IN _comentario VARCHAR(250), IN _id_user_responsable INT, IN _productos_concat VARCHAR(250), IN _cantidad_productos INT)
BEGIN
    ...
    IF @state_validacion = 1 THEN
        ...
        WHILE ( @contador <= _cantidad_productos ) DO
            ...
            -- SELECT @resto;utf8
            --utf8
            -- FALTA PROCESO DE VERIFICARutf8BAJO
            --
...
SET @cantidad_retirar = SPLIT_STR(@producto, '|', 2);
    ...
    IF @cantidad_real < @cantidad_retirar THEN -- VERIFICAMOS STOCK INSUFICIENTE
        ...
        SET @error_mensaje = CONCAT(@error_mensaje,CONCAT(@separador,'STOCK BAJO to RETIRAR ', @cantidad_retirar ,' units. DE (', @id_producto, ':', @nombre_producto, ')'));
...
-- Dump completed on 2022-05-23  0:33:25
